...
- 可选项以每局开关保存，至少支持：
  - `lady_of_lake`：完整规则、状态、AI 决策、前端操作和事件复盘。
  - `tristan_isolde`：在 6-10 人局中把崔斯坦/伊索尔德作为成对身份加入角色池。
...
`tristan_isolde` 开关只允许 6-10 人局启用。启用后，角色池中前两个 `Loyal Servant` 替换为 `Tristan` 和 `Isolde`。5 人局好人名额不足以同时保留 Merlin、Percival 和双人互认组合，因此后端拒绝该组合。

`lady_of_lake` 开关可在 5-10 人局启用，默认关闭。启用时初始持有者为座位最后一位玩家，也就是首任队长右手边玩家；`lady_of_lake_previous_holder_ids` 初始包含该玩家。
...
如果第 3 轮任务后好人已经成功 3 次进入刺杀，或恶方已经失败任务 3 次结束对局，则不再触发湖女阶段。
...
- `enabled_options: list[str] | None`，未知开关拒绝。
...
- `tristan_isolde` 在 5 人局禁用；若请求绕过前端提交，后端仍返回 400。
...
- 规则层：5 人启用 `tristan_isolde` 被拒绝，6-10 人启用后替换两个忠臣。
